<template>
	<view>
		...
		<view class="p-3 flex align-center" >
			<image src="/static/demo/6.jpg" style="height:120rpx;width:120rpx;" class="bg-light rounded-circle flex-shrink" mode="aspectFill"></image>
			<view class="flex flex-column pl-3 flex-1">
				<view class="font-md text-main">点击登录</view>
				<view class="font-sm text-muted">请先登录</view>
			</view>
		</view>
		</navigator>
		<navigator v-else :url="'../user-space/user-space?user_id='+user.id">
		<view class="p-3 flex align-center" >
			<image :src="user.avatar ||'/static/demo/6.jpg'" style="height:120rpx;width:120rpx;" class="bg-light rounded-circle flex-shrink" mode="aspectFill"></image>
			<view class="flex flex-column pl-3 flex-1">
				<view class="font-md text-dark">
					{{user.nickname || user.username}}
					<text class="font-sm text-muted ml-3">{{user.sex}}</text>
				</view>
		...
		<f-list-item icon="iconguanzhu" title="关注" :rightText="followCount"
		@click="navigateTo('user-follow')"></f-list-item>
		<f-list-item icon="iconlishi" title="历史记录"
		@click="navigateTo('user-history')"></f-list-item>
		<!-- #ifdef MP -->
			<f-list-item icon="iconlishi" title="我的设置"
			@click="navigateTo('user-set')"></f-list-item>
		<!-- #endif -->
		<view class="f-divider"></view>
